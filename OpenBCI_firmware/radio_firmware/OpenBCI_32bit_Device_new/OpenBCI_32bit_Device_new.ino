...
void loop(){
  ...
  if(serialTiming){                        // if the serial port is active
    if(millis() - serialTimer > 3){        // time's up! send 'em if you got'em
      if(serialIndex[0]==2){
        //RFduinoGZLL.sendToHost("pt1\n");
        if(serialBuffer[0][1]=='M'){
          //RFduinoGZLL.sendToHost("M found \n");
          domyfunc1();
        }
        /*if(serialBuffer[0][1]=='N'){
          //RFduinoGZLL.sendToHost("N found \n");
          domyfunc2();
        }*/
      }
...
//Old-code
void domyfunc1(){
  //RFduinoGZLL.sendToHost("test1"); //send string to host
  //delay(10);      // wait a little for the packet to be sent before going into ULP
  RFduino_ULPDelay(SECONDS(1)); //1 seconds
  //RFduinoGZLL.sendToHost("test1"); //send string to host
}

void domyfunc2(){
  RFduinoGZLL.sendToHost("test2"); //send string to host
  delay(10);   
  RFduinoGZLL.txPowerLevel = -20;
}

void processCharInput(char* cmdBuffer){
  return;
}

// New Code
/*
// Puts in Low-Power mode for 200ms whenever 'M' is received
void domyfunc1(){
  //RFduinoGZLL.sendToHost("test1"); //send string to host
  //delay(10);      // wait a little for the packet to be sent before going into ULP
  RFduino_ULPDelay(INFINITE); // 200 ms
  RFduino_pinWake(PGCpin, HIGH);
  //RFduinoGZLL.sendToHost("test1"); //send string to host
}

// Not used i.e. 'N' is not sent from OpenBCI board
void domyfunc2(){
  RFduinoGZLL.sendToHost("test2"); //send string to host
  delay(10);   
  RFduinoGZLL.txPowerLevel = -20;
